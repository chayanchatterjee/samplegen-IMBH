...
def get_strain_from_original_file(original_file_paths,
                             gps_time,
                             interval_width,
    ...
    for detector in ('H1', 'L1'):
        ...
        with h5py.File(file_path, 'r') as hdf_file:
            ...
            strain = np.array(hdf_file['H1:GDS-CALIB_STRAIN'])
...
class NoiseTimeline_original:
    ...
    def __init__(self,
        ...
        self.original_file_paths = get_file_paths(self.background_data_directory,
                                             extensions=['hdf', 'h5'])
        self.vprint('Done!')

        # Read in the meta information and masks from original files
        self.vprint('Reading information from hdf files', end=' ')
        self.original_files = self._get_original_files()
        self.vprint('Done!')

        # Build the timeline for these original files
    ...
    def _get_original_files(self):
        ...
        n_files = len(self.original_file_paths)

        for i, original_file_path in enumerate(self.original_file_paths):
...
#                                   dtype=np.int32)
                ...
                original_files.append(dict(file_path=original_file_path,
        ...
        return sorted(original_files, key=lambda _: _['start_time'])
...
#			v1_dq_mask=np.zeros(n_entries, dtype=np.int32))
        ...
        for original_file in self.original_files:
            ...
            detector = original_file['detector']
            dq_mask = original_file['dq_mask']
#            inj_mask = gwf_file['inj_mask']

            # Map start/end from GPS time to array indices
            idx_start = int(original_file['start_time']) - int(self.gps_start_time)
            idx_end = int(idx_start) + int(original_file['duration'])
    ...
    def get_file_paths_for_time(self, gps_time):
        ...
        for original_file in self.original_files:
            ...
            start_time = original_file['start_time']
            end_time = start_time + original_file['duration']
            ...
            if start_time < gps_time < end_time:
                result[original_file['detector']] = original_file['file_path']
    ...
    def gps_start_time(self):
        ...
        return int(self.original_files[0]['start_time'])
    ...
    def gps_end_time(self):
        ...
        return int(self.original_files[-1]['start_time'] + \
            self.original_files[-1]['duration'])
