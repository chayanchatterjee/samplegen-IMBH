...
#                print('Quitting')
            ...
            neglat_seconds=next_task["neglat_seconds"]
            ...
            temp = template_hp.start_time
            template_hp = np.array(template_hp)
            
            if neglat_seconds > 0:
                template_hp[-1*int(neglat_seconds/delta_t):] = np.zeros(int(neglat_seconds/delta_t))
#                temp_strain[-1*int(neglat_seconds/delta_t):] = np.zeros(int(neglat_seconds/delta_t))

            template_time_series = TimeSeries(template_hp,
                delta_t=delta_t, epoch = temp,
                dtype=None, copy=True)
            ...
            results.append(
                {
                    "snr_strain": np.array(snr_series),
                    "mass1": mass1,
                    "mass2": mass2,
                    "spin1z": spin1z,
                    "spin2z": spin2z,
...
class InjectionsBuildFiles(object):
    def __init__(
            ...
            trim_output, inj_low, inj_high, neglat_seconds
    ):
        ...
        self._inj_low = inj_low
        self._inj_high = inj_high
        self._neglat_seconds = neglat_seconds
                    ...
                    tasks.put(
                        {
                            ...
                            "neglat_seconds": self._neglat_seconds
                        }
                    )
                    ...
                    if self._trim_output:
                        if injection_params["det_string"] == "h1_strain":
                            ...
                            omf_h1_data_group.create_dataset(
                                ...
                                data=snr_sample[self._inj_low[injection_params["index"]]:self._inj_high[injection_params["index"]]],
                            )
...
class FiltersBuildFiles(object):
    def __init__(
            ...
            trim_output, inj_low, inj_high, noise_low, noise_high
    ):
        self._output_file_path = output_file_path
        self._df = df
                    ...
                    if self._trim_output:
                        if injection_params['sample_type'] == "injection_samples":
                            if injection_params['det_string'] == "h1_strain":
                                ...
                                v1_injection_data_group.create_dataset(label, 
                                        data=snr_sample[self._inj_low[injection_params["sample_index"]]:self._inj_high[injection_params["sample_index"]]])
                        else:
                            if injection_params['det_string'] == "h1_strain":
                                label = "template" + str(injection_params["template_index"]) + \
                                ...
                                label = "template" + str(injection_params["template_index"]) + \
                                        ",sample" + str(injection_params["sample_index"])
                                print("Creating dataset for V1 - " + label)
                                v1_noise_data_group.create_dataset(label, 
                                        data=snr_sample[self._noise_low[injection_params["sample_index"]]:self._noise_high[injection_params["sample_index"]]])
                    else:
                        if injection_params['sample_type'] == "injection_samples":
                            if injection_params['det_string'] == "h1_strain":
